...
// Motor settings
#define STEPS_PER_REV 650                              // Steps per revolution for the stepper motor (change if needed)
...
void setup()
{
  ...
  Serial.println("Stepper Motor Ready. Send 'run' to start.");
}
...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    if (command == "COMPRESS")
    {
      Serial.println("Command received: Running the motor...");
...
{
  int stepsToMove = rotations * STEPS_PER_REV; // Calculate total steps to move for the giverun n rotations
